...
public class FsmModelStateHelper implements IK3ModelStateHelper{
	...
	public K3ModelState getK3ModelState(EObject model) {
		...
		while (allContentIt.hasNext()) {
			...
			Class<?> clazz =null;
			clazz = K3DslHelper.getTarget(org.gemoc.models17.fsm.semantics.model.aspects.BufferAspect.class);
			if (clazz.isInstance(elem)) {
				ElementState elemState = theFactory.createElementState();
				elemState.setModelElement(elem);
				res.getOwnedElementstates().add(elemState);
				AttributeNameToValue n2v0 = new AttributeNameToValue("currentValues", FsmRTDAccessor.getcurrentValues(elem));
				elemState.getSavedRTDs().add(n2v0);
			}
			clazz = K3DslHelper.getTarget(org.gemoc.models17.fsm.semantics.model.aspects.FSMAspect.class);
			if (clazz.isInstance(elem)) {
				ElementState elemState = theFactory.createElementState();
				elemState.setModelElement(elem);
				res.getOwnedElementstates().add(elemState);
				AttributeNameToValue n2v0 = new AttributeNameToValue("currentState", FsmRTDAccessor.getcurrentState(elem));
				elemState.getSavedRTDs().add(n2v0);
				AttributeNameToValue n2v1 = new AttributeNameToValue("underProcessTrigger", FsmRTDAccessor.getunderProcessTrigger(elem));
				elemState.getSavedRTDs().add(n2v1);
				AttributeNameToValue n2v2 = new AttributeNameToValue("consummedString", FsmRTDAccessor.getconsummedString(elem));
	...
	public void restoreModelState(K3ModelState state) {
		for(ElementState elemState : state.getOwnedElementstates()) {
			for(Object o : elemState.getSavedRTDs()) {
				...
				try {
					if (n2v.value != null) {
						Method m = FsmRTDAccessor.class.getMethod("set"+n2v.name, EObject.class, n2v.value.getClass());
					...
					for(Class<?> c : n2v.value.getClass().getInterfaces()) {
						...
						try {
							m = FsmRTDAccessor.class.getMethod("set"+n2v.name, EObject.class, n2v.value.getClass().getInterfaces()[0]);
							m.invoke(null, elemState.getModelElement(), n2v.value);
